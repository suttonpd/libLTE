...
int cuhd_set_master_clock(void *h, double rate, char *source){
  if(h != NULL)
    return -1;

  char buf[16] = "internal";
  cuhd_handler* handler = static_cast<cuhd_handler*>(h);
  if(source == NULL)
    source = buf;
  handler->usrp->set_master_clock_rate(rate);
  handler->usrp->set_clock_source(source);
  return 0;
}
...
double cuhd_set_rx_srate(void *h, double rate) {
	cuhd_handler* handler = static_cast<cuhd_handler*>(h);
  handler->usrp->set_rx_rate(rate);
